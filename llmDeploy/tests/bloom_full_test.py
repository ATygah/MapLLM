...
try:
    from llmDeploy.models import BLOOM
...
def test_bloom(optimization_level=1, export_traffic=True, type="full", mapping_strategies=None):
    """
    Test the BLOOM model implementation.
    This creates a BLOOM model and runs a simple inference.
    ...
    Args:
        ...
        mapping_strategies: List of mapping strategies to test. If None, only uses "compact"
    """
    try:
        ...
        log_dir = os.path.join(os.path.dirname(__file__), "../final_traces/bloom/full_test")
        ...
        log_file = os.path.join(log_dir, f"bloom_{type}_test_opt{optimization_level}_{timestamp}.log")
        ...
        if mapping_strategies is None:
            mapping_strategies = ["compact"]
        
        # Create test config path
        config_path = os.path.abspath(os.path.join(os.path.dirname(__file__), "../../config/models/bloom_175b.yaml"))
        ...
        with open(log_file, "w") as f:
            ...
            f.write(f"===== Testing BLOOM Model (Optimization Level {optimization_level}) =====\n\n")
            ...
            try:
                ...
                test_config_path = os.path.join(log_dir, f"bloom_test_config_{timestamp}.yaml")
            ...
            for strategy in mapping_strategies:
                ...
                try:
                    # Create a BLOOM model with the current mapping strategy
                    f.write(f"Creating BLOOM model with optimization level {optimization_level} and mapping strategy '{strategy}'...\n")
                    print(f"Creating BLOOM model with optimization level {optimization_level} and mapping strategy '{strategy}'...")
                    ...
                    print(f"BLOOM model created successfully with {strategy} strategy")
                    f.write(f"BLOOM model created successfully with {strategy} strategy.\n\n")
                    ...
                    if hasattr(model, 'llm') and hasattr(model.llm, 'noc') and hasattr(model.llm.noc, 'scheduler'):
                        ...
                        if not traffic_table.empty:
                            ...
                            #generate_network_layout_visualization(model.llm, f)
                            ...
                            if export_traffic:
                                ...
                                if not enhanced_traffic_table.empty:
                                    ...
                                    enhanced_traffic_file = os.path.join(
                                        log_dir, 
                                        f"bloom_traffic_enhanced_{type}_{strategy}_opt{optimization_level}_{timestamp}.tsv"
                                    )
                                    ...
                                    traffic_file = os.path.join(
                                        log_dir, 
                                        f"bloom_traffic_{type}_{strategy}_opt{optimization_level}_{timestamp}.tsv"
                                    )
            ...
            if len(mapping_strategies) > 1:
                ...
                # for strategy, assessment in assessments.items():
                #     if strategy in mapping_strategies:
                #         f.write(f"{strategy}: {assessment}\n")
                
                # Generate visualizations
                # f.write("\n\n===== Visualization Plots =====\n\n")
                plot_dir = os.path.join(log_dir, "plots")
                # plot_prefix = f"bloom_{type}_opt{optimization_level}"
        ...
        print(f"Unhandled exception in test_bloom: {e}")
        traceback.print_exc()
        return None, {}
...
if __name__ == "__main__":
    try:
        ...
        #mapping_strategies = ["row_wise", "column_wise", "grid_wise", "compact", "proximity"]
        mapping_strategies = ["grid_wise"]
        
        results, plot_files = test_bloom(optimization_level=1, export_traffic=True, type="full", mapping_strategies=mapping_strategies)
