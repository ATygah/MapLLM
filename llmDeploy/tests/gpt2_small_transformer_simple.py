...
def test_multihead_attention_simple():
    ...
    log_dir = os.path.join(os.path.dirname(__file__), "../final_traces/gpt2_small/transformer/row_seq512")
    os.makedirs(log_dir, exist_ok=True)
    log_file = os.path.join(log_dir, f"gpt2_small_transformer_simple_{timestamp}.log")
    ...
    with open(log_file, "w") as f:
        ...
        execution_order.append(["mlp1"])
        execution_order.append(["mlp2"])
        ...
        # Connect output projection to MLP1 and MLP1 to MLP2
        llm.connect_networks("output_proj", "mlp1")
        llm.connect_networks("mlp1", "mlp2")
        f.write(f"Connected output_proj -> mlp1 -> mlp2\n")
        print(f"Connected output_proj -> mlp1 -> mlp2")
        ...
        try:
            ...
            try:
                timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
                traffic_filename = f"gpt2_small_transformer_simple_{timestamp}.txt"
                #traces_dir = os.path.join(os.path.dirname(__file__), "../final_traces/gpt2_small/transformer")
                export_path = export_traffic_table_to_file(llm, traffic_filename, directory=log_dir)
